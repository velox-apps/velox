...
struct DeferredEchoResponse: Codable, Sendable {
  let message: String
  let delayMs: Int
}
...
struct DelayedEchoArgs: Codable, Sendable {
  let message: String
  let delayMs: Int?
}
...
func main() {
  ...
  let registry = commands {
    ...
    command("delayed_echo", args: DelayedEchoArgs.self, returning: DeferredCommandResponse.self) { args, context in
      let deferred = try context.deferResponse()
      let delay = max(0, args.delayMs ?? 800)
      DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(delay)) {
        deferred.responder.resolve(DeferredEchoResponse(message: args.message, delayMs: delay))
      }
