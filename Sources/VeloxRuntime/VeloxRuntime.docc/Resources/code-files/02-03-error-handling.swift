...
let registry = commands {
    ...
    // Division can throw an error for division by zero
    command("divide", args: MathArgs.self, returning: MathResponse.self) { args, _ in
        guard args.b != 0 else {
            throw CommandError(code: "DivisionByZero", message: "Cannot divide by zero")
        }
        return MathResponse(result: args.a / args.b, operation: "division")
    }
}
